...
def run(params, bed, g):
    ...
    alignment = pysam.AlignmentFile(
        params['falignment']['path'],
        params['falignment']['mode']
    )
    ...
    for n in nodes:
        ...
        if g.graph.nodes[n]['fusion'].first.is_init():
            sel = bed.apply(
                _select_bed,
                axis=1,
                args=(g.graph.nodes[n]['fusion'].first,)
            )
            sub_first = bed[sel]
        if g.graph.nodes[n]['fusion'].second.is_init():
            sel = bed.apply(
                _select_bed,
                axis=1,
                args=(g.graph.nodes[n]['fusion'].second,)
            )
            sub_second = bed[sel]
        ...
        if len(sub_first) + len(sub_second) == 2:
            logging.warning(
                'Fusion %s is found on left and right of breakpoint in \
                    the bed -> skipping' % (
                    g.graph.nodes[n]['fusion'],
                )
            )
        ...
        if len(sub_first) + len(sub_second) == 0:
            logging.warning(
                "Fusion %s isn't found on left or right of breakpoint in \
                    the bed -> skipping" % (
                    g.graph.nodes[n]['fusion'],
                )
            )
        ...
        for aligned_segment in alignment.fetch(
            ...
            # Filtering.
            if (
                aligned_segment.is_unmapped or
                aligned_segment.is_duplicate or
                aligned_segment.is_supplementary
            ):
                continue

            cigar2pos = _cigar2position(
                aligned_segment.cigartuples,
            ...
            if aligned_segment.is_paired:
                if (
                    not aligned_segment.mate_is_unmapped and
